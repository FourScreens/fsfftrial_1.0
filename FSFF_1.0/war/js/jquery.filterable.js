(function($) {
	$.fn.filterable = function(opts) {
		...
		var allMatches = function(row) {
			return $(row).find('td.filterable-no-match').length === 0;
		};
		var setFilterActive = function(element) {
			$(element).addClass('filterable-active');
		};
		var clearFilter = function(element) {
			$(element).removeClass('filterable-active');
		};
		...
		var addFilter = function(heading, index) {
			...
			var icon = ' <i class="icon-filter filterable" data-name="' + index
					+ '" title="' + title + '"></i>';
