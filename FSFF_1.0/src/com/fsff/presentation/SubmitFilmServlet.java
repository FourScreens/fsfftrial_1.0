...
public class SubmitFilmServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*FilmUploadService film = new FilmUploadService();
		film.setName(request.getParameter("FilmName"));
		film.setCategory(request.getParameter("Category"));
		film.setCast(request.getParameter("Cast"));
		film.setTwoMin(request.getParameter("twomin"));
		film.setFiveMin(request.getParameter("fivemin"));
		film.setTenMin(request.getParameter("tenmin"));
		uploadFilm(film);
		*/
		...
		String roundThreeClip = request
				.getParameter("roundThreeClip");
		int userId = 0;
		if(request.getSession().getAttribute("JSESSION")!=null){
			FilmSubmissionService filmSubmissionObject = new FilmSubmissionService();
			UserSession userSession = (UserSession)	request.getSession().getAttribute("JSESSION");
			userId = userSession.getId();
			filmSubmissionObject.submitFilm(filmName, genre, filmDescription, roundOneClip, roundTwoClip, roundThreeClip, userId);
